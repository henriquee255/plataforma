...
const CRM = () => {
  ...
  // Carregar pipelines do localStorage ou inicializar com dados mockados
  useEffect(() => {
    const savedPipelines = localStorage.getItem('crm_pipelines');
    const savedActivePipelineId = localStorage.getItem('crm_activePipelineId');

    if (savedPipelines) {
      try {
        const parsed = JSON.parse(savedPipelines);
        setPipelines(parsed);
        if (savedActivePipelineId) {
          setActivePipelineId(savedActivePipelineId);
        } else if (parsed.length > 0) {
          setActivePipelineId(parsed[0].id);
        }
        // toast.success('Dados do CRM carregados!', 2000);
        return; // Se carregou do localStorage, não precisa dos dados mockados
      } catch (error) {
    ...
    // Se não tem dados salvos, inicializar com mockados
    const initialLeads = [
      {
        id: 1,
        nome: 'João Silva',
    ...
    setLeads(initialLeads); // Manter compatibilidade

    // Inicializar notas mockadas
    setLeadNotes({
      1: [
        { id: 1, text: 'Cliente demonstrou muito interesse na proposta premium', date: '2026-02-23 10:30', author: 'Maria Santos' }
      ],
      2: [
        { id: 1, text: 'Agendada reunião para amanhã às 14h', date: '2026-02-22 16:20', author: 'Carlos Mendes' },
        { id: 2, text: 'Cliente pediu prazo até sexta-feira', date: '2026-02-21 11:15', author: 'Carlos Mendes' }
      ]
    });
  }, []);

  // Salvar pipelines no localStorage automaticamente
  useEffect(() => {
    if (pipelines.length > 0) {
      try {
        localStorage.setItem('crm_pipelines', JSON.stringify(pipelines));
        if (activePipelineId) {
          localStorage.setItem('crm_activePipelineId', activePipelineId);
        }
      } catch (error) {
